...
class UsuarioDetalhesRepository
{
    ...
    public function add(string $email, string $objetivo, ?string $google_drive, string $segmento, ?string $instagram, ?string $ajudante, $localizacao)
    {
        ...
        $stmt = $conn->prepare('CALL USUARIO_DETALHES_CONTROLLER(:acao, :param_email, :param_objetivo, :param_google_drive, :param_segmento, :param_instagram, :param_ajudante, :param_localizacao)');
        $stmt->execute([
            ':acao' => 'add',
            ':param_email' => $email,
