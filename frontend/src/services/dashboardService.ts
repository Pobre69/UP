...
export type DashboardRange = 'diario' | 'semanal' | 'mensal';

export const dashboardService = {
  async getDashboardData(range: DashboardRange = 'mensal'): Promise<DashboardData> {
    const response = await fetchWithRetry(
      `${API_BASE_URL}/api/dashboard?range=${encodeURIComponent(range)}`,
      {
        method: 'GET',
        credentials: 'include',
